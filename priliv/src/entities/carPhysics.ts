...
export const NO_MODS: CarMods = { engine: false, tires: false, armor: false };

/** Engine: +18% top speed and +25% acceleration. Tyres: +25% grip. */
export function moddedSpec(spec: CarSpec, mods: CarMods): CarSpec {
  if (!mods.engine && !mods.tires) return spec;
  return {
    ...spec,
    maxSpeed: spec.maxSpeed * (mods.engine ? 1.18 : 1),
    accel: spec.accel * (mods.engine ? 1.25 : 1),
    grip: spec.grip * (mods.tires ? 1.25 : 1),
    handbrakeGrip: spec.handbrakeGrip * (mods.tires ? 1.15 : 1),
  };
}

/** Armour soaks 40% of incoming damage. */
export function armorFactor(mods: CarMods): number {
  return mods.armor ? 0.6 : 1;
}
